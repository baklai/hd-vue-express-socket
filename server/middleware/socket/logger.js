...
module.exports = (socket, unless) => {
    return async ([event, ...args], next) => {
        ...
        const log = await Logger.create({
            ...
            event: event,
            datetime: socket?.handshake?.time ? new Date(socket.handshake.time).toLocaleString() : new Date().toLocaleString(),
            agent: socket?.handshake?.headers['user-agent'] ? socket.handshake.headers['user-agent'] : 'anonymous'
        });
        console.log(`${log.address} [${log.user}] - ${eventStr(log.event)} [${log.datetime}] "${log.agent}"`);
        return next();
    };
};

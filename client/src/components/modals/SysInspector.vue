...
defineExpose({
  toggle: async ({ id }) => {
    try {
      ...
      await IPAddress.findOne({ ipaddress: Inspector.record.host, populate: true });
      visible.value = true;
    } catch (err) {
      visible.value = false;
        ...
        <div class="flex justify-content-between mb-4">
          <div class="flex justify-content-cente w-4">
            <div class="flex flex-column card-container">
              <div class="flex align-items-center justify-content-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="40" height="40" class="text-color">
                  <path
                    fill="currentColor"

...
defineExpose({
  toggle: async ({ id }) => {
    try {
      if (id) record.value = await Unsoftware.findOne({ id });
      else record.value = Unsoftware.$reset();
      records.value = await Unsoftware.findAll({});
      visible.value = true;
    } catch (err) {
      visible.value = false;
...
const onSaveRecord = async () => {
  ...
  if (valid) {
    if (record.value?.id) {
      await Unsoftware.updateOne(record.value);
      toast.add({
        severity: 'success',
        summary: t('HD Information'),
      ...
      await Unsoftware.createOne(record.value);
      toast.add({
        severity: 'success',
        summary: t('HD Information'),
...
<template>
  ...
  >
    <template #header>
      <div class="flex justify-content-between w-full">
        <div class="flex align-items-center justify-content-center">
          <i class="pi pi-microsoft text-6xl mr-3"></i>
          <div>
            <p class="text-lg font-bold line-height-2 mb-0">{{ $t('Unwanted software') }}</p>
            ...
            <small class="font-normal line-height-2 text-color-secondary">
              {{ $t('Unwanted software from database') }}
            </small>
          </div>
        </div>
    ...
    <form @submit.prevent="onSaveRecord" class="p-fluid">
      ...
      <div class="field">
        <label for="comment">{{ $t('Software comment') }}</label>
        <Textarea
          rows="5"
          id="comment"
          class="min-w-full"
